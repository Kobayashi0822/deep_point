...
def model(minibatch_size = 32, num_epochs = 8, learning_rate = 0.001, mode = "TRAIN", restore_saved_model = False, save_every_n_epochs = 1, log_folder = '1'):
	...
	loss = custom_loss(pred = predictions, y = y)
	tf.summary.scalar('custom_loss', loss) # For tensorboard logging.
	...
	save_path = "C:/Users/CURRENT-WORKING-DIRECTORY/SAVE_PATH_CUSTOM_LOSS_ENTIRE/chkpts/model.ckpt" # Change this directory to wherever you want to save the training checkpoints.
	save_path_lowest = "C:/Users/CURRENT-WORKING-DIRECTORY/SAVE_PATH_CUSTOM_LOSS_ENTIRE/best_chkpt/model.ckpt" # This is the path where the parameters, resulting in the lowest loss yet, will be saved.

	batch_gen = BatchLoader(batch_size = 32,
			...
			total_num_data_points = 312) # Batch generator initialization.
		...
		with tf.Session() as sess:
			...
			while batch_gen.epochs_passed != num_epochs:            # |
				...
				for i in range(num_slices): # *IMPORTANT* - THIS WILL NEGLECT THE LAST 'NUM_DATA_POINT % MINIBATCH_SIZE' EXAMPLES.
					...
					print(i, loss_val, end = "\r") # For monitoring.
					if i % 32 == 0:
	...
	if mode == "VALIDATE":
		...
		with tf.Session() as sess:
			...
			X_val, y_val = batch_gen.get_validation_batch(remove_stray = False) # Loads validation batch.
...
def main():
	model(minibatch_size = 16, num_epochs = 500, learning_rate = 0.0001, mode = "TRAIN", restore_saved_model = True, log_folder = '2') # Uncomment this for training, while commenting out the line below.
